	...
	public static String getTimeRemaining(Player player, String type, int cooldownMillis) {
		if (cooldown.containsKey(player.getName() + "-" + type)) {
			if (60000 > (cooldownMillis - (System.currentTimeMillis() - cooldown.get(player.getName() + "-" + type)))) {
				int seconds = Math.round((cooldownMillis - (System.currentTimeMillis() - cooldown.get(player.getName() + "-" + type))) / 1000);
				return seconds + " seconds";
			}
			else {
				int minutes = Math.round((cooldownMillis - (System.currentTimeMillis() - cooldown.get(player.getName() + "-" + type))) / 60000);
				int seconds = Math.round((cooldownMillis - (System.currentTimeMillis() - cooldown.get(player.getName() + "-" + type))) / 1000) - (minutes * 60);
				if (minutes > 1) {

...
public class PVP implements Listener, CommandExecutor
{
	...
	public static void onEntityDamageByEntity(EntityDamageByEntityEvent e)
	{
		...
		if (victim instanceof Player)
		{
			...
			if (attacker instanceof Projectile)
			{
				...
				{
					if (projectile instanceof Arrow || projectile instanceof Egg || projectile instanceof FishHook || projectile instanceof Snowball)
					{
						e.setCancelled(true);
					}
				}
	...
	public static void onPotionSplash(PotionSplashEvent e)
	{
		if (e.getEntity().getShooter() instanceof Player)
		{
			...
			for (LivingEntity entity : e.getAffectedEntities())
			{
				if (entity instanceof Player && !(PVP.getPvp((Player) entity) && PVP.getPvp((Player) e.getEntity().getShooter())))
				{
					Cooldown.reset((Player)entity, "pvp", 5000);
					PotionEffectType[] pvpPotions = {PotionEffectType.HARM, PotionEffectType.POISON, PotionEffectType.SLOW, PotionEffectType.WEAKNESS};
